...
def get_exif(picFile):
  ...
  try:
      ...
      print('IOERROR:',picFile)


def get_pic_propery(picFile,exif):
    ...
    if gps!=None:
        ...
        latitude_d = (lat[0][0]*1.0)/lat[0][1] + ((lat[1][0]*1.0)/lat[1][1])/60.0 + ((lat[2][0]*1.0)/lat[2][1])/3600.0
    ...
    # Picture Create Time
    createTime = exif['DateTimeOriginal'] if 'DateTimeOriginal' in exif.keys() else "1970:01:01 23:59:59"
    ...
    if re.match('^[0-9]{4}:[0-9]{1,2}:[0-9]{1,2} [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}$', createTime):
        ...
        createTime_T = time.localtime(os.path.getmtime(picFile))

    pic_info['createTime'] = createTime_T

    #LensModel
    lensModel = exif['LensModel'] if 'LensModel' in exif.keys() else 'unknown'
    pic_info['lensModel'] = lensModel
